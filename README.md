...
- **Login is magic-link only — no passwords anywhere.** `POST /auth/request-link` finds-or-creates the `User` by email; that's also how a brand-new owner "signs up" (there's no separate registration endpoint). Creating the Organisation/Venue itself is Epic 1 step 5, not auth.
